  ...
  patents: {
    ...
    async analyze(id) {
      ...
      try {
        ...
        let finalStatus = (isDuplicate || analysis.decisionRecommendation === 'REJECT') ? 'Rejected' : 'Formality Check Pending';
        ...
        fresh[fi] = {
          ...
          ai_report:             analysis,
          similarity_risk:       isDuplicate || (analysis.similarPatents || []).some(s => s.similarityScore >= 65),
          status:    finalStatus,
        ...
        logActivity(fresh[fi].patent_id, fresh[fi].title,
          isDuplicate
            ? `AI: REJECTED — duplicate of ${analysis.duplicateOf}. ${analysis.duplicateReason}`
            : `AI analysis complete — novelty: ${analysis.noveltyScore}, strength: ${analysis.strengthScore}, decision: ${analysis.decisionRecommendation}`
        );

        // Auto-run formality check if analysis passed
        if (!isDuplicate && analysis.decisionRecommendation !== 'REJECT') {
          try {
            ...
            const formalityStatus = formalityResult.passed ? 'Examiner Review' : 'Requires Modification';
            const finalFresh = store.get(KEYS.PATENTS);
            const finalIdx = finalFresh.findIndex(p => String(p.id) === String(id));
            finalFresh[finalIdx] = { 
              ...finalFresh[finalIdx], 
            ...
            store.set(KEYS.PATENTS, finalFresh);
            logActivity(finalFresh[finalIdx].patent_id, finalFresh[finalIdx].title,
              `Formality check ${formalityResult.passed ? 'PASSED' : 'FAILED'} (automatic) (score: ${formalityResult.score}) — ${formalityResult.summary}`
            );
            finalStatus = formalityStatus;
          } catch (formalityErr) {
            console.error('Formality check error:', formalityErr.message);
            logActivity(fresh[fi].patent_id, fresh[fi].title, `Formality check error: ${formalityErr.message}`);
          }
        }
    ...
    async examinerCheck(id) {
      const patent = store.get(KEYS.PATENTS).find(p => String(p.id) === String(id));
      if (!patent) throw new Error('Patent not found');

      // Run AI examiner review
      const result = await callAI('examiner-check', { patent });

      const newStatus = result.approved ? 'Approved' : 'Rejected';
      const patents = store.get(KEYS.PATENTS);
      const idx = patents.findIndex(p => String(p.id) === String(id));
      patents[idx] = { ...patents[idx], status: newStatus, updated_at: new Date().toISOString() };
      store.set(KEYS.PATENTS, patents);
      logActivity(patents[idx].patent_id, patents[idx].title,
        `AI Examiner Review: ${result.approved ? 'APPROVED' : 'REJECTED'} — ${result.reason || result.summary}`
      );
